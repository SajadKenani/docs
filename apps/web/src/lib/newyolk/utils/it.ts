...
import type { ITPageProps } from '../types/IT'
import { allITs } from 'contentlayer/generated'
import { getSlugWithoutLocale } from './locale'
import { defaultLocale } from '@/config/i18n'
import { itConfig } from '@/config/it'

export function makeLocalizedSlug({ locale, slug }: ITPageProps['params']) {
...
export async function getITFromParams({
  params,
}: ITPageProps): Promise<
  ((typeof allITs)[0] & { notAvailable: boolean }) | null
> {
  let localizedSlug = makeLocalizedSlug(params)
  let doc = allITs.find((doc) => doc.slugAsParams === localizedSlug)
  ...
  if (!doc) {
    ...
    doc = allITs.find((doc) => doc.slugAsParams === localizedSlug)
...
export function getBreadcrumb(docSlug: string) {
  ...
  const breadcrumbs = makeBreadcrumb(slug, itConfig)
