...
import type { allITs } from 'contentlayer/generated'
...
import type { IT } from 'contentlayer/generated'
...
export async function getPagerForCurrentDoc({
  ...
  const itConfig = await getServerITConfig({ locale })
  const flattenedLinks = [null, ...flatten(itConfig.it.sidebarNav), null]

  const slugWithoutLocaleFolder = getSlugWithoutLocale(doc.slug, 'it')
  ...
  return {
    prev,
    next,
    currentLocale: itConfig.currentLocale,
  }
}
