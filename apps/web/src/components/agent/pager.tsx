...
import type { allAgents } from 'contentlayer/generated'
...
export async function getPagerForCurrentDoc({
  ...
  const agentConfig = await getServerAgentConfig({ locale })
  const flattenedLinks = [null, ...flatten(agentConfig.agent.sidebarNav), null]

  const slugWithoutLocaleFolder = getSlugWithoutLocale(doc.slug, 'agent')
  ...
  return {
    prev,
    next,
    currentLocale: agentConfig.currentLocale,
  }
}
